...
public class InputManager : MonoBehaviour
{
    ...
    private void Awake() => playerActions = new PlayerActions();
    ...
    private void Start()
    {
        playerActions.Touch.PrimaryContact.started += ctx => StartTouchPrimary(ctx);
        playerActions.Touch.PrimaryContact.canceled += ctx => EndTouchPrimary(ctx);
        playerActions.Touch.Tap.performed += ctx => TappedPerformed(ctx);
        playerActions.Touch.MultiTap.performed += ctx => MultiTappedPerformed(ctx);
    }
    ...
    private Vector2 ScreenPosition() { return playerActions.Touch.PrimaryPosition.ReadValue<Vector2>(); }
